...
export class DashboardService {
  ...
  private async daysStatBuilder(type: StatType): Promise<StatGroup[]> {
    ...
    switch (type) {
      case StatType.User:
        result = await Promise.all(range.map(async date => ({
      ...
      case StatType.Online:
        result = await Promise.all(range.map(async date => ({
          date: this.moment(date).utc().toDate(),
          amount: (await this.onlinesRecordsRepository.findOne({
            created: Between(this.moment(date).toDate(), this.moment(date).endOf('day').toDate())
          }))?.online || 0
        })))
        break;
      case StatType.Purchase:
        result = await Promise.all(range.map(async date => ({
          ...
          amount: Number((await this.historyRepository
            .createQueryBuilder()
      ...
      case StatType.Payment:
        result = await Promise.all(range.map(async date => ({
            ...
            amount: Number((await this.paymentsRepository
              .createQueryBuilder()
